...
fn get_min_distance<'a>(
    ray: &props::ray::Ray,
    objects: &'a Vec<Box<dyn Drawable>>,
) -> (Option<&'a Box<dyn Drawable>>, Option<f64>) {
...
fn render_pixel_on_image(
    ...
    let (obj_to_render, curr_dist_min): (Option<&Box<dyn Drawable>>, Option<f64>) =
        get_min_distance(&light_ray, &scene.objects);
    ...
    match obj_to_render {
        Some(obj_to_render) => {
            ...
            let color_to_render =
                compute_light(&light_ray, obj_to_render, scene, curr_dist_min.unwrap());
...
fn compute_light(
    ...
    for light in &scene.lights {
        ...
        // Shadow computation.
        let shadow_ray = props::ray::Ray::new(
            light_ray_hit + surface_normal * 1e-4,
            direction_to_light_normal,
        );
        ...
        let light_intensity = match dst_to_near_obj {
            Some(d) => {
                if d > light.distance(light_ray_hit) {
                    light.intensity(light_ray_hit)
                } else {
                    0.0
                }
            }
